...
namespace Pet_caring_website.DTOs.Pet
{
    public class PetDto
    {
        ...
        // hinting that this should be a date (not a datetime, string, etc.).
        [DataType(DataType.Date)]
        [CustomValidation(typeof(PetDto), nameof(ValidateAdoptDate))]
        public DateTime? AdoptDate { get; set; }

        public static ValidationResult? ValidateAdoptDate(DateTime? adoptDate, ValidationContext context)
        {
            if (adoptDate.HasValue && adoptDate > DateTime.Now)
            {
                return new ValidationResult("AdoptDate cannot be in the future.");
            }

            // null is okay, so success
