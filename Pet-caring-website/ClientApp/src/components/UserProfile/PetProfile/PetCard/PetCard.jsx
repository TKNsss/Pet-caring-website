...
const PetCard = () => {
  ...
  const specieStatus = useSelector((state) => state.species.status);
  ...
  const getInitialFormData = (selectedPet) => ({
    ...
    adoptDate: selectedPet?.adopt_date
      ? selectedPet?.adopt_date.slice(0, 10)
      : "",
    age: selectedPet?.age_in_months || "",
    spcId: selectedPet?.spc_id || "",
  ...
  useEffect(() => {
    if (specieStatus === "idle") dispatch(fetchSpecies());
  }, [dispatch, specieStatus]);
  ...
  const handleUpdate = async () => {
    if (!selectedPet) {
      toast.warning("Please select a Pet to udpate!");
      return;
    }
    ...
    if (
      ...
      toast.warning(
        "Please filled out at least name, age, weight, and genders!",
      );
      return;
    }
    ...
    try {
      await dispatch(
        updateSinglePet({ petId: selectedPet.pet_id, petData: newFormData }),
      ).unwrap();
      resetForm("clear");
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await dispatch(deletePet(selectedPet.pet_id)).unwrap();
      setShowModal(false);
      resetForm("clear");
    } catch (err) {
          ...
          {petFields
            ...
            .map((field) => {
              ...
              const verifyInputType = () => {
                if (field.name === "weight" || field.name === "age")
                  return "number";
                ...
                >
                  ...
                  {isSelect ? (
                    ...
                    <input
                      ...
                      required={["age", "weight"].includes(field.name)}
