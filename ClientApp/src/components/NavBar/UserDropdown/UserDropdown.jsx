...
const UserToggleSection = ({ open, links, user, scrolled, onLogout }) => {
  return (
    <div className="font-Poppins relative">
      {open && (
        <div
          className={`absolute -right-3 z-50 mt-11 w-51 rounded-lg transition-colors duration-300 @3xl:mt-9 ${
            scrolled
              ? "bg-lavender/90 shadow-lg backdrop-blur-sm"
              : "bg-lavender"
          }`}
        >
          ...
          <div className="border-navBorder border-b p-4 text-center">
            <h3 className="text-txt-2 font-semibold">
...
const UserDropdown = ({ user, scrolled }) => {
  ...
  return (
    <>
      ...
      >
        ...
        <MdKeyboardArrowDown
          className={`text-lg transition duration-150 ease-out ${open ? "rotate-0" : "-rotate-180"} `}
        />
      </div>
